/*
 * /ajax.cgi Query-Parser, unveraendert vom Original uebernommen.
 * Einzige Aenderungen: das eigene atoi() entfernt (kollidiert mit der
 * newlib), uri ist const, Anbindung an esp_http_server statt uIP.
 */
...
static void decodeString(char *dest, const char *src, unsigned int len) {
    char c;
    while ((c = *src++) && len--) {
        if (c == '%') {
...
static int parseUri(const char *uri, int len, uriParse_t *ps) {
...
static unsigned char nextParam(char *paramPtr, char *valuePtr, unsigned char len, uriParse_t *u) {
...
static unsigned char firstParam(char *paramPtr, char *valuePtr, unsigned char len, uriParse_t *u) {
    idx = 0;
                    ...
                    if (strncmp(value, "setTime", 7) == 0) {
                        ...
                        while (nextParam(param, value, BUF-1, &u)) {
                            if (strncmp(param, "id", 2) == 0) {
                                ...
                                strncpy(timer.name, value, sizeof(timer.name)-1);
