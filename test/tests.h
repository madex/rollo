 ...
 * Release 0.31
 *
 * 08.02.2010 MO Added ASSERT_EQUALS_IN_RANGE for 2 integers witch are tested in a epsilon range
                 ASSERT_EQUALS_IN_RANGE(2, 1, 1) do not assert but
				 ASSERT_EQUALS_IN_RANGE(2, 4, 1) will assert
 * 27.06.2013 MO added newline and tab fail fail reason description.
 *
 ...
 *
 */
#ifndef SIMPLECTEST_TESTS_H
#define SIMPLECTEST_TESTS_H
...
static int val_a, val_b;

// --- Assertion commands ---
#define ASSERT_EQUALS(a, b) \
	{TEST("\n\t" #a " is supposed to equal " #b); \
	val_a = (int) a; val_b = (int) b; \
	sprintf(info, "\n\t%d(0x%X) is not == %d(0x%X)", val_a, val_a, val_b, val_b); \
	ASSERT(val_a == val_b);}

// --- Assertion commands ---
#define ASSERT_EQUALS_IN_RANGE(a, b, epsilon) \
        {TEST("\n\t" #a " is supposed to equal " #b " with epsilon +- " #epsilon); \
	val_a = (int) a; val_b = (int) b; \
	sprintf(info, "\n\t%d(0x%X) is not == %d(0x%X)", val_a, val_a, val_b, val_b); \
	ASSERT(val_a >= val_b - epsilon && val_a <= val_b + epsilon);}

// Asserts that a != b
#define ASSERT_NOT_EQUALS(a, b) \
	{TEST("\n\t" #a " is not supposed to equal " #b); \
	val_a = (int) a; val_b = (int) b; \
	sprintf(info, "\n\t%d(0x%X) is not != %d(0x%X)", val_a, val_a, val_b, val_b); \
	ASSERT(val_a != val_b);}
